...
class UserInput(Player):
    ...
    def __init__(self, player_name, topic, is_multiple_answers_allowed=False):
        super().__init__(None, player_name)
        self.question_template = """This player is under construction and cannot be used. However, we can mock its result :)
    What would you expect the player \"{player_name}\" response to be about the {topic}?
        q - to quit """
    ...
    def execute(self):
        questionTemplate = self.question_template.format(
            player_name=self.player_name, topic=self.topic)
